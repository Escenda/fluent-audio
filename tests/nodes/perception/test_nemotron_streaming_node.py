...
def _decode_transcript_outputs(fake_node: FakeDoraNode):
    ...
    return deltas, finals, stream_final
...
def test_nemotron_streaming_node_emits_transcripts_and_stream_final_marker() -> None:
    ...
    deltas, finals, stream_final = _decode_transcript_outputs(fake_node)
...
def test_nemotron_streaming_node_rejects_audio_input_closed_without_final_marker() -> None:
    ...
    with pytest.raises(NemotronStreamingNodeError, match="before explicit audio final marker"):
        run_nemotron_streaming_events(fake_node, _config(), CountingBackend())


def test_nemotron_streaming_node_rejects_control_after_control_input_closed() -> None:
    fake_node = FakeDoraNode(
        [
    ...
    with pytest.raises(NemotronStreamingNodeError, match="after control input closed"):
        run_nemotron_streaming_events(fake_node, _config(), CountingBackend())
