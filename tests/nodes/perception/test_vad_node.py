...
from nodes.perception.vad.main import VadNodeConfig, VadNodeError, run_vad_events
...
def _decode_activity_outputs(fake_node: FakeDoraNode):
    ...
    for output_id, payload, metadata in fake_node.sent:
        ...
        if metadata["final"]:
            ...
            activity_events.append(decode_voice_activity_event_from_dora(payload, metadata))
    assert final_marker is not None
    return activity_events, final_marker
...
def test_vad_dora_node_emits_speech_events_and_final_marker() -> None:
    ...
    activity_events, final_marker = _decode_activity_outputs(fake_node)
...
def test_vad_dora_node_reports_padded_flush_as_window_span() -> None:
    ...
    assert [event.frame_count for event in activity_events] == [512, 512]
    assert final_marker.sample_index == 768


def test_vad_dora_node_final_partial_window_uses_evaluation_window_span() -> None:
