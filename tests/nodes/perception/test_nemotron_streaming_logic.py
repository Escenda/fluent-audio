...
def _config(*, prebuffer_frames: int = 2048) -> NemotronStreamingConfig:
...
def test_rejects_audio_format_source_stream_and_sequence_mismatch() -> None:
    ...
    discontinuity_runtime = NemotronStreamingRuntime(_config(), CollectingBackend())
    discontinuity_runtime.push_audio(_chunk(seq=0, sample_index=0))
    with pytest.raises(NemotronStreamingError, match="discontinuity"):
        discontinuity_runtime.push_audio(_chunk(seq=2, sample_index=512))
