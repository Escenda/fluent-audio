...
from nodes.vad.turn_detector.main import (
...
def test_turn_detector_node_accepts_final_marker_inside_last_padded_activity_window() -> None:
    fake_node = FakeDoraNode(
        [
            _dora_activity_event(
                _activity(seq=0, sample_index=0, state="speech", probability=0.9)
            ...
            _dora_activity_event(
                _activity(seq=1, sample_index=512, state="silence", probability=0.1)
            ),
            _dora_activity_final_event(seq=2, sample_index=832),
        ]
    )

    summary = run_turn_detector_events(fake_node, _config(end_silence_frames=512))
    turn_events, final_marker = _decode_turn_outputs(fake_node)

    assert summary.final_sample_index == 832
    assert final_marker.sample_index == 832
    assert [event.state for event in turn_events] == ["started", "ended"]
