...
use fluent_audio_io_boundary::{
    capture_time_ns_for_frame_offset, i16_samples_to_s16le_bytes, AudioFormat, AudioMetadata,
    ChannelLayout, SampleFormat, AUDIO_OUTPUT_ID,
};
use thiserror::Error;
...
fn run(config: CaptureConfig) -> Result<(), CaptureError> {
    ...
    loop {
        ...
        let parameters = metadata.to_dora_parameters()?;
        node.send_output_bytes(output_id.clone(), parameters, payload.len(), &payload)
            .map_err(CaptureError::DoraOutput)?;
        seq = metadata.next_seq();
        sample_index = metadata.next_sample_index();
        chunks_sent += 1;
    }
    ...
    node.send_output_bytes(output_id, final_metadata.to_dora_parameters()?, 0, &[])
        .map_err(CaptureError::DoraOutput)?;
    Ok(())
}
