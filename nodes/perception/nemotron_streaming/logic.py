        ...
        if self._chunks:
            try:
                ...
                raise NemotronStreamingError(
                    "Audio history received non-contiguous audio"
                ) from exc
    ...
    def span(self, start_sample_index: int, end_sample_index: int) -> list[AudioChunk]:
        ...
        for chunk in self._chunks:
            ...
            if sliced.sample_index != next_sample_index:
                raise NemotronStreamingError(
                    "Audio history span is not contiguous at sample_index "
                    f"{next_sample_index}"
                )
...
class NemotronStreamingRuntime:
    ...
    def __init__(
        ...
        self._history = AudioHistoryBuffer(max_frames=config.prebuffer_frames)
    ...
    def push_audio(self, chunk: AudioChunk) -> list[TranscriptDelta | TranscriptFinal]:
        self._validate_audio_chunk(chunk)
        self._history.append(chunk)
        self._previous_audio_chunk = chunk
        if self._active_turn is None:
            return []
        return self._push_active_audio_from_chunk(chunk)

    def push_control(
        self,
        control: AsrStart | AsrStop | AsrCancel,
    ) -> list[TranscriptDelta | TranscriptFinal]:
    ...
    def finish_audio(self, final_sample_index: int) -> None:
        ...
        if self._active_turn is not None:
            raise NemotronStreamingError(
                "ASR audio stream finished while a turn is still active"
            )

    def _start_turn(self, control: AsrStart) -> list[TranscriptDelta]:
        if self._active_turn is not None:
        ...
        latest_sample_index = self._history.latest_sample_index
        if latest_sample_index is None or control.start_sample_index >= latest_sample_index:
            return []

        replay_chunks = self._history.span(
            control.start_sample_index,
            latest_sample_index,
        )
        transcript_events: list[TranscriptDelta] = []
        for chunk in replay_chunks:
            transcript_events.extend(self._push_backend_audio(chunk))
        return transcript_events

    def _stop_turn(self, control: AsrStop) -> list[TranscriptDelta | TranscriptFinal]:
    ...
    def _push_active_audio_from_chunk(
        ...
        if active_turn is None:
            return []
        if chunk.next_sample_index <= active_turn.next_sample_index:
            return []
        if active_turn.next_sample_index < chunk.sample_index:
            raise NemotronStreamingError(
                "ASR active turn has an audio gap: "
                f"next={active_turn.next_sample_index}, chunk={chunk.sample_index}"
            )

        target_end_sample_index = chunk.next_sample_index
        if active_turn.pending_stop is not None:
        ...
        chunk_to_push = slice_audio_chunk(
            chunk,
            start_sample_index=active_turn.next_sample_index,
            end_sample_index=target_end_sample_index,
        )
        transcript_events: list[TranscriptDelta | TranscriptFinal] = []
        transcript_events.extend(self._push_backend_audio(chunk_to_push))
        ...
        if active_turn is not None and active_turn.pending_stop is not None:
            if active_turn.next_sample_index == active_turn.pending_stop.stop_sample_index:
                transcript_events.append(self._finalize_active_turn(active_turn.pending_stop))
        return transcript_events

    def _push_backend_audio(self, chunk: AudioChunk) -> list[TranscriptDelta]:
        active_turn = self._active_turn
        if active_turn is None:
    ...
    def _finalize_active_turn(self, control: AsrStop) -> TranscriptFinal:
        active_turn = self._require_active_turn(control.session_id, control.user_turn_id)
    ...
    def _validate_audio_chunk(self, chunk: AudioChunk) -> None:
