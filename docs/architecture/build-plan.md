...
Current state: repository scaffold, agreed directory structure, contracts, raw PCM source/sink DORA boundaries, the offline DORA roundtrip dataflow, CPAL capture/sink hardware smokes, the GStreamer-backed `media_graph`, the Silero VAD DORA node, and the deterministic `turn_detector` are green. The `nemotron_streaming` node has verified DORA/control/prebuffer orchestration and explicit NeMo backend configuration validation, but remains yellow until the actual NeMo/Nemotron backend is installed and smoke-tested on target hardware. Later runtime nodes remain yellow.
...
flowchart TD
    ...
    class repo_scaffold,directory_structure,contracts,raw_pcm_source,raw_pcm_sink,offline_roundtrip_dataflow,cpal_capture,cpal_sink,media_graph,vad,turn_detector verified;
    class nemotron_streaming,dialogue_engine,codex_app_server,tts_backend,playback_queue,ros2_bridge,web_session_projection scaffold;
```
...
| `contracts` | `src/fluent_audio/contracts` | Green: Pydantic v2 contracts implemented for audio chunks, activity/turn events, ASR control, transcripts, dialogue/agent events, synthesis chunks, playback state, session correlation, and DORA boundary helpers for audio/activity/turn/ASR/transcript. | Payload length, `seq`/sample continuity, capture time, format mismatch, serialized dump/validate roundtrip without computed helper fields, zero-frame audio span rejection, bounded probabilities/confidence, discriminated command/control variants, DORA metadata final markers, and correlation ids are verified. | `uv run --extra dev --extra dora python -m pytest tests/contracts`; `uv run --extra dev --extra dora python -m ruff check src/fluent_audio/contracts src/fluent_audio/dora tests/contracts` |
...
| `turn_detector` | `nodes/perception/turn_detector` | Green: DORA node consumes typed voice activity events, validates source/stream/continuity/completion, deterministically emits `started`/`active`/`ended` turn events, flushes active turns on completion, sends a typed turn final marker, and passes VAD-to-turn DORA smoke. | Consumes activity context and emits typed turn boundary candidates with deterministic transition fixtures. | `uv run --extra dev --extra dora --extra vad python -m pytest tests/contracts tests/nodes/perception`; `uv run --extra dev --extra dora --extra vad python -m ruff check .`; `uvx --from dora-rs-cli dora run dataflows/turn_detector_smoke.yml --uv` |
| `nemotron_streaming` | `nodes/perception/nemotron_streaming` | Yellow: ASR DORA boundary, audio/control event loop, strict 16 kHz mono `s16le` validation, bounded prebuffer replay, pending-stop handling, transcript delta/final emission, transcript stream final marker, and explicit `--backend nemo` configuration validation are implemented and unit-tested. Actual NeMo/PyTorch backend construction and real Nemotron model smoke are not implemented, so this is not green. | Streams audio to Nemotron 3.5 ASR Streaming 0.6B through a real NeMo backend, emits true transcript deltas/finals, and passes target-hardware smoke with the downloaded model. | Current partial verification: `uv run --extra dev --extra dora python -m pytest tests/nodes/perception/test_nemotron_streaming_backend.py tests/nodes/perception/test_nemotron_streaming_logic.py tests/nodes/perception/test_nemotron_streaming_node.py`; `uv run --extra dev --extra dora python -m ruff check nodes/perception/nemotron_streaming tests/nodes/perception/test_nemotron_streaming_backend.py tests/nodes/perception/test_nemotron_streaming_logic.py tests/nodes/perception/test_nemotron_streaming_node.py`. Green still requires NeMo/PyTorch install, model download, and real-model streaming smoke. |
