...
Current state: repository scaffold, agreed directory structure, contracts, raw PCM source, and raw PCM sink are green. DORA dataflow and later runtime implementation are not green yet.
...
flowchart TD
    ...
    class repo_scaffold,directory_structure,contracts,raw_pcm_source,raw_pcm_sink verified;
    class offline_roundtrip_dataflow,cpal_capture,cpal_sink,media_graph,vad,turn_detector,nemotron_streaming,dialogue_engine,codex_app_server,tts_backend,playback_queue,ros2_bridge,web_session_projection scaffold;
```
...
| `raw_pcm_source` | `nodes/io/sources/raw_pcm_source`, `src/fluent_audio/offline/raw_pcm.py` | Green: headerless raw PCM source logic and CLI smoke wrapper are implemented and verified. Task: [raw-pcm-io-implementation-task.md](raw-pcm-io-implementation-task.md). Subagent prompt: [raw-pcm-io-subagent-prompt.md](raw-pcm-io-subagent-prompt.md). | Reads headerless PCM with explicit format, emits ordered `AudioChunk` payloads including final partial chunks, and rejects empty input or frame-size mismatch. | `uv run --extra dev python -m pytest tests/contracts tests/nodes/io`; `uv run --extra dev python -m ruff check src/fluent_audio/offline nodes/io/sources/raw_pcm_source nodes/io/sinks/raw_pcm_sink tests/nodes/io` |
| `raw_pcm_sink` | `nodes/io/sinks/raw_pcm_sink`, `src/fluent_audio/offline/raw_pcm.py` | Green: headerless raw PCM sink logic and CLI smoke wrapper are implemented and verified. Task: [raw-pcm-io-implementation-task.md](raw-pcm-io-implementation-task.md). Subagent prompt: [raw-pcm-io-subagent-prompt.md](raw-pcm-io-subagent-prompt.md). | Accepts explicit-format `AudioChunk`, rejects source/stream/format/sequence/sample-index mismatch, rejects empty streams, and writes exact PCM bytes. | `uv run --extra dev python -m pytest tests/contracts tests/nodes/io`; `uv run --extra dev python -m ruff check src/fluent_audio/offline nodes/io/sources/raw_pcm_source nodes/io/sinks/raw_pcm_sink tests/nodes/io` |
| `offline_roundtrip_dataflow` | `dataflows` | Yellow: dataflow directory scaffold only. Task: [raw-pcm-io-implementation-task.md](raw-pcm-io-implementation-task.md). Subagent prompt: [raw-pcm-io-subagent-prompt.md](raw-pcm-io-subagent-prompt.md). | Wires source to sink with explicit format and queue policy; fixture PCM roundtrips byte-for-byte through DORA. | `dora run dataflows/offline_roundtrip.yml --uv`; `cmp tests/fixtures/offline/input.s16le artifacts/offline/output.s16le` |
