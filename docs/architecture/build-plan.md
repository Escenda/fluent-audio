...
| `contracts` | `src/fluent_audio/contracts` | Yellow: package scaffold only. Implementation task: [contracts-implementation-task.md](contracts-implementation-task.md). | Typed contracts required by the first vertical slice exist with boundary validation: audio chunks, continuity, VAD/turn events, transcript delta/final, dialogue events, synthesis chunks, and playback state. | `uv run --extra dev python -m pytest tests/contracts`; `uv run --extra dev python -m ruff check .` |
