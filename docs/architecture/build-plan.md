...
Current state: repository scaffold, agreed directory structure, contracts, raw PCM source/sink DORA boundaries, the offline DORA roundtrip dataflow, CPAL capture/sink hardware smokes, the GStreamer-backed `media_graph`, the Silero VAD DORA node, and the deterministic `turn_detector` are green. The `nemotron_streaming` node has verified DORA/control/prebuffer orchestration, but remains yellow until the actual NeMo/Nemotron backend is installed and smoke-tested on target hardware. Later runtime nodes remain yellow.
...
| `nemotron_streaming` | `nodes/perception/nemotron_streaming` | Yellow: ASR DORA boundary, audio/control event loop, strict 16 kHz mono `s16le` validation, bounded prebuffer replay, pending-stop handling, transcript delta/final emission, and transcript stream final marker are implemented and unit-tested with a deterministic backend. CLI backend wiring and actual NeMo/Nemotron model smoke are not implemented, so this is not green. | Streams audio to Nemotron 3.5 ASR Streaming 0.6B through a real NeMo backend, emits true transcript deltas/finals, and passes target-hardware smoke with the downloaded model. | Current partial verification: `uv run --extra dev --extra dora python -m pytest tests/nodes/perception/test_nemotron_streaming_logic.py tests/nodes/perception/test_nemotron_streaming_node.py`; `uv run --extra dev --extra dora python -m ruff check nodes/perception/nemotron_streaming tests/nodes/perception/test_nemotron_streaming_logic.py tests/nodes/perception/test_nemotron_streaming_node.py`. Green still requires NeMo/PyTorch install, model download, and real-model streaming smoke. |
