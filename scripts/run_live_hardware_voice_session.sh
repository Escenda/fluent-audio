...
if [[ "${MODE}" != "--serve" && "${MODE}" != "--run" && "${MODE}" != "--write-dataflow" ]]; then
  echo "usage: $0 [--serve|--run|--write-dataflow]" >&2
  exit 64
fi
...
if [[ "${MODE}" != "--write-dataflow" && "${FLUENT_AUDIO_ALLOW_LIVE_CODEX_TURN:-}" != "1" ]]; then
...
CODEX_HOME_DIR="${FLUENT_AUDIO_CODEX_HOME:-${REPO_ROOT}/artifacts/codex_home/live_hardware_voice_session}"
mkdir -p "${CODEX_HOME_DIR}"
export CODEX_HOME="${CODEX_HOME_DIR}"

LIVE_RUNTIME_LOG="${FLUENT_AUDIO_LIVE_RUNTIME_LOG:-artifacts/live_hardware_voice_session/runtime.log}"
...
LIVE_DATAFLOW="${FLUENT_AUDIO_LIVE_HARDWARE_DATAFLOW:-graphs/out/live_hardware_voice_session.local.yml}"
SESSION_ID="${FLUENT_AUDIO_LIVE_HARDWARE_SESSION_ID:-live-hardware-session}"
...
python - "${LIVE_DATAFLOW}" "${MODE}" "${SESSION_ID}" "${WEB_BRIDGE_PORT}" "${CODEX_CONTROL_PORT}" "${TTS_PYOPENJTALK_PORT}" "${VLLM_BASE_URL}" "${VLLM_MODEL}" "${VLLM_PROVIDER}" "${NEMOTRON_MODEL_RESOLVED}" "${NEMOTRON_VENV_WRAPPER_RESOLVED}" "${LIVE_RUNTIME_LOG_RESOLVED}" <<'PY'
...
def input_device_args() -> list[str]:
    use_default = os.environ.get("FLUENT_AUDIO_USE_DEFAULT_INPUT_DEVICE") == "1"
    device_name = os.environ.get("FLUENT_AUDIO_CPAL_INPUT_DEVICE_NAME")
    device_id = _device_id_or_default(
        env_name="FLUENT_AUDIO_CPAL_INPUT_DEVICE_ID",
...
nemotron_model_path = sys.argv[10]
nemotron_venv_wrapper = sys.argv[11]
runtime_log_path = sys.argv[12]
...
input_backend = os.environ.get("FLUENT_AUDIO_INPUT_BACKEND", "cpal")
if input_backend not in ("cpal", "alsa_pcm", "pipewire_pcm"):
    raise ValueError("FLUENT_AUDIO_INPUT_BACKEND must be cpal, alsa_pcm, or pipewire_pcm")
capture_max_chunks = os.environ.get("FLUENT_AUDIO_CAPTURE_MAX_CHUNKS")
...
capture_sample_rate_hz = os.environ.get(
    "FLUENT_AUDIO_CAPTURE_SAMPLE_RATE_HZ",
    default_capture_sample_rate_hz,
)
capture_channels = os.environ.get("FLUENT_AUDIO_CAPTURE_CHANNELS", "1")
...
if input_backend == "cpal":
    capture_args = [
        *input_device_args(),
        ...
        "interleaved",
        "--chunk-frames",
        os.environ.get("FLUENT_AUDIO_CAPTURE_CHUNK_FRAMES", default_capture_chunk_frames),
        "--buffer-size-frames",
        os.environ.get("FLUENT_AUDIO_CAPTURE_BUFFER_SIZE_FRAMES", "2048"),
        "--queue-capacity-chunks",
        os.environ.get("FLUENT_AUDIO_CAPTURE_QUEUE_CAPACITY_CHUNKS", "2048"),
        "--source-id",
        capture_source_id,
        "--stream-id",
        capture_stream_id,
        ...
        "0",
        "--capture-timeout-ms",
        os.environ.get("FLUENT_AUDIO_CAPTURE_TIMEOUT_MS", "3000"),
    ]
    ...
    capture_args = [
        ...
        "--dora",
        "--device",
        os.environ.get("FLUENT_AUDIO_ALSA_CAPTURE_DEVICE", "pipewire"),
        "--sample-rate-hz",
        capture_sample_rate_hz,
        "--channels",
        capture_channels,
    ...
    capture_args = [
        ...
        "--target",
        os.environ.get(
            "FLUENT_AUDIO_PIPEWIRE_CAPTURE_TARGET",
            "alsa_input.usb-Anker_PowerConf_S3_A3321-DEV-SN1-01.mono-fallback",
        ),
        "--latency",
        os.environ.get("FLUENT_AUDIO_PIPEWIRE_CAPTURE_LATENCY", "20ms"),
        "--sample-rate-hz",
        capture_sample_rate_hz,
        "--channels",
        capture_channels,
...
lines.extend(
    ...
    + yaml_args(
        [
            ...
            "--warmup-frames",
            os.environ.get("FLUENT_AUDIO_NEMOTRON_WARMUP_FRAMES", "16000"),
            "--sample-rate-hz",
            "16000",
            "--channels",
            "1",
            ...
            nemotron_model_path,
            "--target-lang",
            os.environ.get("FLUENT_AUDIO_ASR_TARGET_LANG", "ja-JP"),
    ...
    [
        ...
        + yaml_args(
            [
                *output_device_args(),
                ...
                os.environ.get("FLUENT_AUDIO_OUTPUT_STARTUP_BUFFER_CHUNKS", "2"),
                "--empty-queue-policy",
                os.environ.get("FLUENT_AUDIO_OUTPUT_EMPTY_QUEUE_POLICY", "silence"),
                "--source-id",
                "speaker_media_graph",
                "--stream-id",
                "speaker/cpal",
        ...
        + yaml_args(
            [
                ...
                "--runtime-log",
                runtime_log_path,
                *(["--keep-http-after-dora-stop"] if mode == "--serve" else []),
            ]
        ),
        "    inputs:",
        "      activity:",
...
if [[ "${FLUENT_AUDIO_INPUT_BACKEND:-alsa_pcm}" == "pipewire_pcm" ]]; then
  PIPEWIRE_CAPTURE_TARGET="${FLUENT_AUDIO_PIPEWIRE_CAPTURE_TARGET:-alsa_input.usb-Anker_PowerConf_S3_A3321-DEV-SN1-01.mono-fallback}"
